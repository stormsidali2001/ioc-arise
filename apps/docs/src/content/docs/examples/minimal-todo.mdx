...
The complete dependency graph for the Todo application:
...
classDiagram
    ...
    ITodoRepository <|.. InMemoryTodoRepository : implements
    TodoService --> ITodoRepository : depends on
```

**Dependency Flow:**
- `TodoService` implements `ITodoService` and depends on `ITodoRepository`
- `InMemoryTodoRepository` implements `ITodoRepository`
- `Todo` is a domain entity with no dependencies
...
// ✅ Knows ITodoService needs ITodoRepository
...
CLI analyzed your TypeScript files → Detected `TodoService` depends on `ITodoRepository` → Generated all wiring code automatically → Created type-safe resolution
...
✅ Autocomplete for all services • ✅ Type errors at compile-time • ✅ IDE refactoring support
...
<CardGrid>
  <Card title="✅ Zero Decorators" icon="approve-check">
    Pure TypeScript code with no framework coupling. Your domain logic stays clean!
  </Card>
  <Card title="🎯 Interface-Based" icon="star">
  ...
  <Card title="⚡ Auto-Wiring" icon="rocket">
