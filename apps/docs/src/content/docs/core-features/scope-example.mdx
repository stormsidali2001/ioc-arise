...
<CardGrid>
  <Card title="🔄 Singleton (Default)" icon="star">
    **One instance shared everywhere**
    
    • Perfect for stateless services  
    • Better performance (created once)  
    • Use for: Repositories, configs, caches  
    • No annotation needed (default)
  </Card>
  
  <Card title="⚡ Transient" icon="rocket">
    **New instance every time**
    
    • Perfect for stateful services  
    • Isolated state per request  
    • Use for: Request handlers, commands  
    • Requires `@scope transient` annotation
  </Card>
</CardGrid>

## Class Structure

Services can have different lifecycle scopes, controlled by JSDoc annotations:
...
classDiagram
    ...
    SingletonService --> ILogger : depends on
    TransientService --> ILogger : depends on
    ScopedService --> ILogger : depends on
```

**Scope Differences:**
- `SingletonService`: Default scope (singleton) - one instance shared everywhere
- `TransientService`: `@scope transient` - new instance every time
- `ScopedService`: Explicit `@scope singleton` - same as default but explicit
...
```typescript
// ✅ No @scope annotation = Singleton by default
...
// ✅ Logger: Singleton (default)
container.register('ILogger', {
  useClass: Logger,
  lifecycle: Lifecycle.Singleton,
});

// ✅ SingletonService: Singleton (default, no annotation)
...
// ✅ TransientService: Transient (from @scope transient)
...
// ✅ ScopedService: Singleton (from @scope singleton)
...
• Detect `@scope` JSDoc tags  
• Generate correct lifecycle configurations

## When to Use Each Scope

<CardGrid>
  <Card title="🔄 Use Singleton For:" icon="star">
  ...
  <Card title="⚡ Use Transient For:" icon="rocket">
...
<CardGrid>
  <Card title="🎯 Simple Annotations" icon="pencil">
