...
<CardGrid>
  <Card title="👥 User Module" icon="user">
    **User Domain** - Encapsulates all user-related functionality:
    ...
    • `UserRepository` in-memory implementation  
    • `IUserService` business logic interface  
    • `UserService` with CRUD operations
  </Card>
  
  <Card title="✅ Todo Module" icon="document">
    **Todo Domain** - Manages todo items with user ownership:
    
    • `Todo` entity with user reference  
    • `ITodoRepository` data access interface  
    • `TodoRepository` in-memory implementation  
    • `ITodoService` business logic interface  
    • `TodoService` **depends on UserModule!**
  </Card>
</CardGrid>

## Class Structure

The module system organizes classes into logical groups with cross-module dependencies:
...
classDiagram
    ...
    class ITodoService {
        <<interface>>
        +createTodo(title, userId) Promise~Todo~
        +getTodosByUser(userId) Promise~Todo[]~
    }
    class TodoService {
    ...
    ITodoRepository <|.. TodoRepository : implements
    TodoService --> ITodoRepository : depends on
    TodoService ..> IUserRepository : cross-module dependency
```

**Key Pattern:**
- `UserModule` contains user-related services and repositories
- `TodoModule` contains todo-related services and repositories
- `TodoService` depends on `IUserRepository` from `UserModule` (cross-module dependency)

## Cross-Module Dependencies

:::note[🔗 The Power of Modules]
...
:::tip[✨ One Command, Modular Output]
...
└── modules/
    ├── userModule.module.ts  # User module
    └── todoModule.module.ts  # Todo module
```
...
:::tip[🎯 Type-Safe Cross-Module Resolution]
...
// ❌ Try to create todo for non-existent user
try {
...
export class Todo {
  constructor(
    public id: string,
    public title: string,
    public userId: string,  // ✅ Links to User
...
export interface ITodoService {
  ...
  getTodosByUser(userId: string): Promise<Todo[]>;  // ✅ Cross-module query
...
:::note[⚙️ Simple Module Configuration]
...
**What this does:**
• Groups all `user/` files → `UserModule`  
• Groups all `todo/` files → `TodoModule`  
• Resolves cross-module dependencies automatically  
• Generates separate module files with proper order

## Key Takeaways

<CardGrid>
  <Card title="📦 Modular Organization" icon="puzzle">
    Keep related code together in logical, self-contained modules. Perfect for domain-driven design!
  </Card>
  <Card title="🔗 Cross-Module DI" icon="star">
  ...
  <Card title="🧪 Easy Testing" icon="approve-check">
  ...
  <Card title="✅ Auto-Validation" icon="warning">
