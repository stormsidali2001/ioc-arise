...
:::danger[⚠️ Module Circular Dependencies]
...
The module-level circular dependency creates an unsolvable cycle:
...
classDiagram
    ...
    IServiceA <|.. ServiceA : implements
    IServiceB <|.. ServiceB : implements
    ServiceA --> IServiceB : depends on
    ServiceB --> IServiceA : depends on
```

**The Cycle:** `ModuleA` (via `ServiceA`) → `ModuleB` (via `IServiceB`) → `ModuleA` (via `IServiceA`) → **impossible to load either module!**

## IoC Arise Detection

:::tip[🔍 Automatic Module-Level Detection]
...
- ✅ **Clear error message** - shows exact module dependency chain
- ✅ **File locations** - know exactly which modules are involved
- ✅ **Prevents deployment** - won't generate container with broken module dependencies

## Key Takeaways

<CardGrid>
  <Card title="🔍 Module-Level Detection" icon="magnifier">
    IoC Arise detects circular dependencies **between modules**, not just classes.
  </Card>
  <Card title="⚡ Compile-Time Safety" icon="rocket">
...
:::caution[💡 Architectural Consideration]
